...
**Nada** del agente sale de este directorio. No se tocan reglas de negocio ni el InputMap. Si el contexto no está en la escena ni en el pedido, el playtester devuelve `NEED_SETUP` a quien lo llamó. La acción es `press` de una acción ya mapeada, o un click cuyo puntero pega en el control. Si no hay acción, es un bug.
...
| `crash_side.json` | Fixture: cubo ancho a la izquierda, `move_left` → **Chocaste** |
