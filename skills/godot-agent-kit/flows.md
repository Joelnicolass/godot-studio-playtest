...
`scene` vacío en la raíz = main scene. Preferí `"scene": "res://agent/fixtures/…"` para una feature aislada; la main solo si el criterio es boot/hub. Un step `{ "scene": "res://…" }` cambia a mitad del flow. `click` dispara `BaseButton.pressed` (no hit-test de píxel) — es el botón que el jugador pulsaría, no un `call` al handler. `press` es **solo** una acción que ya existe en el InputMap: `"ui_accept"` o `{ "name": "move_left", "hold": 0.4 }`. Si no está mapeada, el step falla con `unmapped input` (bug de producto). No mandes `KEY_*` ni un keycode. `%Nombre` se resuelve en la escena y, si falta, en hijos. `try_click` no falla si el nodo falta, está `disabled` o no está visible (`AGENT_SKIP`). `repeat` corre `steps` hasta `times` o hasta que `until` (mismo shape que `assert`) pase.
