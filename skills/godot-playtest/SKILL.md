...
description: >-
  ...
  AgentKit flows or harnesses. Never put playtest helpers in src/.
---
...
If `addons/agent_kit/` exists: `inspect --unique`, write the flow in `res://agent/flows/`. Helpers only in `res://agent/harness/` ([harness.md](../godot-agent-kit/harness.md)). The harness **may** `call()` methods the product already has, including `_prefixed` ones.
